...
namespace RaidAtAGlance
{
    ...
    public static class Patch_Letter_DrawButtonAt
    {
        ...
        public static void Postfix(Letter __instance, float topY)
        {
            ...
            {
                RaidAtAGlanceMod.RaidInfo info = null;
                if (RaidAtAGlanceMod.raidInfoHistory.ContainsKey(__instance))
                {
                    info = RaidAtAGlanceMod.raidInfoHistory[__instance];
                }
                else
                {
                    bool hasTargetInRaid = RaidAtAGlanceMod.raidInfo != null && __instance.lookTargets != null && __instance.lookTargets.targets.Any(t => t.Pawn != null && RaidAtAGlanceMod.raidInfo.pawns.Contains(t.Pawn));
                    if (hasTargetInRaid)
                    {
                        info = RaidAtAGlanceMod.raidInfo;
